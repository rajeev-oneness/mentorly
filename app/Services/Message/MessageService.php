...
class MessageService
{
    ...
    public function __construct()
    {
        $this->pusher = new Pusher(
            config('message.pusher.key'),
            config('message.pusher.secret'),
            config('message.pusher.app_id'),
            config('message.pusher.options'),
        );
    }
    /**
    ...
    public function getAllowedImages(){
        return config('message.attachments.allowed_images');
    }
    ...
    public function getAllowedFiles(){
        return config('message.attachments.allowed_files');
    }
     ...
     * @return array
     */
    public function fetchMessage($id, $guard = null){
        $attachment = null;
        ...
        // dd($user);
        $msg = MessageLog::where('id',$id)->first();
        ...
        return [
            ...
            'message_from_guard' => $msg->message_from_guard,
            'message_to' => $msg->message_to,
            'message_to_guard' => $msg->message_to_guard,
            'message' => $msg->message,
            'attachment' => [$attachment, $attachment_title, $attachment_type],
            'time' => $msg->created_at->diffForHumans(),
            'fullTime' => $msg->created_at,
            // 'viewType' => ($msg->message_from == $user->id && $msg->message_from_guard == $guard) ? 'sender' : 'default',
            'viewType' => ($msg->message_from == $user->id) ? 'sender' : 'default',
            'seen' => $msg->seen,
        ];
    }
    ...
    public function messageCard($data, $viewType = null){
        ...
        return view('messages.layouts.messageCard',$data)->render();
    }
     ...
     * @return Collection
     */
    public function fetchMessagesQuery($user_id, $guard = null){
        $userGuard = get_guard();
        // dd($userGuard);
        $user = Auth::guard($userGuard)->user();
        // dd($user->id);
        // $messages =  MessageLog::where('message_from_guard', $userGuard)
        //                 ->where('message_to_guard', $guard)
        //                 ->where('message_from', $user->id)
        //                 ->where('message_to', $user_id)
        //                 ->orwhere('message_from_guard', $guard)
        //                 ->where('message_to_guard', $userGuard)
        //                 ->where('message_from', $user_id)
        //                 ->where('message_to', $user->id);
        return MessageLog::where('message_from',$user->id)->where('message_to',$user_id)
                    ->orWhere('message_from',$user_id)->where('message_to',$user->id);
        // dd($messages->first());
    ...
    public function newMessage($data){
        ...
        $message->id  = $data['id'];
        $message->message_from  = $data['message_from'];
        $message->message_from_guard  = $data['message_from_guard'];
        $message->message_to  = $data['message_to'];
        $message->message_to_guard  = $data['message_to_guard'];
     ...
     * @return bool
     */
    public function makeSeen($user_id, $user_guard=null){
        $guard = get_guard();
        $user = Auth::guard($guard)->user();
        // dd($user->id);
        MessageLog::Where('message_from',$user_id)
                ->where('message_to', $user->id)
                ->where('seen',0)
                ->update(['seen' => 1]);
        return 1;
    ...
    public function countUnseenMessages($user_id){
        $guard = get_guard();
        $user = Auth::guard($guard)->user();
        return MessageLog::where('message_from',$user_id)->where('message_to',$user->id)->where('seen',0)->count();
    ...
    public function getContactItem($messenger_id, $user){
        ...
        return view('messages.layouts.listItem', [
            'get' => 'users',
            'user' => $user,
